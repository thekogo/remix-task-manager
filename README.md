...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create task-manager-2-c10e
  fly apps create task-manager-2-c10e-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app task-manager-2-c10e
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app task-manager-2-c10e-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'task-manager-2-c10e-staging' does not match app name in config file 'task-manager-2-c10e'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name task-manager-2-c10e-db
  fly postgres attach --app task-manager-2-c10e task-manager-2-c10e-db

  fly postgres create --name task-manager-2-c10e-staging-db
  fly postgres attach --app task-manager-2-c10e-staging task-manager-2-c10e-staging-db
  ```
